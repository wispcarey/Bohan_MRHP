function [p,hk] = ExpcstepTemp_recur(mu,A,omega,H,deltat,h,Lam)
  %calculate g(t_i-t_j)
  ...
  topics=H(:,1)';
  N=length(tdata);
  P=omega*A(topics,topics).*triu(exp(-omega*deltat),1).*(h(Lam)'); 
  ...
  At=A';
  T=tdata(end);
  hk=sum(log(deno))-sum(sum(bsxfun(@etimes,(1-exp(-omega*(T-tdata))).*h(Lam),At(:,topics))))-T*sum(mu);
end
